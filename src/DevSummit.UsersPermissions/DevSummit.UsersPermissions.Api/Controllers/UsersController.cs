...
public class UsersController : ControllerBase
{
    ...
    public ActionResult<UserDto> GetById(string id)
    {
        ...
        return new UserDto(user.Name, user.Email, (int) user.HasAccess);
    }
    ...
    public ActionResult<string> Post([FromBody] UserDto user)
    {
        ...
        var result = service.AddUser(new User { Name = user.Name, Email = user.Email, HasAccess = (Permissions) user.HasAccess });
        if (!result.IsValid)
        {
    ...
    public ActionResult<string> Put(string id, [FromBody] UserDto user)
    {
        ...
        var result = service.UpdateUser(new User { Id = Guid.Parse(id), Name = user.Name, Email = user.Email, HasAccess = (Permissions) user.HasAccess });
        if (!result.IsValid)
        {
            logger.LogError(result.Message);    
...
public record UserDto(string Name, string Email, int HasAccess);
